...
class RetinaNet(M.Module):
    ...
    def __init__(self, cfg, batch_size):
        ...
        # ----------------------- build the backbone ------------------------ #
        bottom_up = resnet18(norm=layers.get_norm(self.cfg.resnet_norm))

        # ------------ freeze the weights of resnet stage1 and stage 2 ------ #
        if self.cfg.backbone_freeze_at >= 1:
            for p in bottom_up.conv_bn_relu1.parameters():
    ...
    def preprocess_image(self, image):
        return layers.get_padded_tensor(image, 32, 0.0)
