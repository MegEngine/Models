...
def main():
    ...
    parser.add_argument(
        "-m", "--mode",
        default="normal", type=str, help="normal qat")
    parser.add_argument(
        "-r", "--restore",
        default=None, type=str, help="model you wanna restore from"
    )
...
def worker(rank, world_size, args):
    ...
    if args.restore is not None and os.path.exists(args.restore):
        logger.info("Load model: {}".format(args.restore))
        ckpt_state = mge.load(args.restore)
        model.load_state_dict(ckpt_state["state_dict"])

    if args.mode == "qat":
        Q.quantize_qat(model, qconfig=Q.ema_fakequant_qconfig)
        logger.info("Set to quant mode.")
    ...
    # HACK: for keep learning rate in scheduler
    # FIXME whenever finding elegant solution
    for group in optimizer.param_groups:
        group.setdefault("initial_lr", group["lr"])

    scheduler = optim.MultiStepLR(optimizer, [30, 60, 80], current_epoch=-1 if ckpt_state is None else ckpt_state["epoch"] - 1)
    ...
    # Start training
    top1_acc = 0
    start_epoch = 0 if ckpt_state is None else ckpt_state["epoch"]
